...
export const PAYMENT_SORTERS: Sorters = {
...
export const getFilters = (): Filter[] => {
  ...
  if (saved) {
    const savedFilterOptions = PAYMENT_FILTER_OPTIONS.map(filter => {
...
export const getSorters = (): Sorters => {
  ...
  return {
    applied: savedSorter || PAYMENT_SORTERS.applied,
    options: PAYMENT_SORTERS.options
  }
}
