      ...
      <div class="w-full flex flex-col max-h-screen overflow-auto pt-2">
        ...
        {#if status === 'active'}
          <div class="my-4 flex flex-col items-center justify-center">
            <Qr values={[{label: $translate('app.labels.bolt12'), value: `lightning:${bolt12}`.toUpperCase()}]} />
            {#if offerExpiry}
              <div class="mt-2">
                <ExpiryCountdown
            ...
            >
              ...
              {#if status === 'active' || status === 'completed'}
                <div
                  ...
                  class="w-4 ml-1 border border-utility-success rounded-full"
                >
                  {@html check}
                </div>
              {:else}
                <div in:fade|local={{ duration: 250 }} class="w-4 ml-1">
                  {@html warning}
                </div>
              {/if}
            </span>
