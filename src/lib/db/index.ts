...
class DB extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      payments:
        '&[id+walletId], timestamp, status, direction, data.channel.type, [data.channel.id+walletId], data.offer.id, network, [walletId+type], data.payIndex, *metadata.tags, metadata.contact, data.fallbackAddress, data.amount, data.fee, [type+status]',
      tags: '&id, label',
      trades:
